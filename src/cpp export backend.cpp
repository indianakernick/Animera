...
Error CppExportBackend::writeCpp() {
  ...
  stream << '\n';
  stream << "#include \"" << atlasName + ".hpp\"\n";
  stream << '\n';
  stream << "const std::size_t animera::" << nameSpace << "::texture_size = " << textureBuffer.size() << ";\n";
  stream << '\n';
  stream << "const unsigned char animera::" << nameSpace << "::texture_data[] = {";
  stream.flush();
